...
from api.schemas.transaction import TransactionQueryParam, ExpenseResponse, ExpenseRequest, ExpenseResponseWithBudgetFeedback
from api.services.expense_service import create_expense_service, delete_expense_service, get_expense_service, update_expense_service
...
@router.post('/create', status_code=status.HTTP_201_CREATED, response_model=ExpenseResponseWithBudgetFeedback)
...
@router.put('/update/{expense_id}', status_code=status.HTTP_200_OK, response_model=ExpenseResponseWithBudgetFeedback)
def update_expense(
    expense_id: int,
