...
@router.put("/update", status_code=status.HTTP_200_OK, response_model=UserProfile)
def update_profile(userProfileRequest : UserProfile, user : dict = Depends(require_roles("user", "admin")), db : Session = Depends(get_db)):
